...
app.post('/api/documents/generate', async (req, res) => {
  try {
    const { templateId, records, fields, customInstruction } = req.body;
    // Custom template: use user's instruction directly
    if (templateId === 'custom') {
      if (!customInstruction) return res.status(400).json({ error: 'Custom instruction required' });
      const aiResp = await llmChat([
        { role:'system', content:'You are a professional document generator. Return valid HTML for the requested document. Use h1, h2, p, table, ul, li tags. Include inline CSS for a clean, professional look. No markdown fences.' },
        { role:'user', content: `Generate a document: ${customInstruction}\n\nData to use:\n${JSON.stringify(records.slice(0, 10))}\n\nFields: ${(fields||[]).join(', ')}\n\nGenerate a complete, styled HTML document.` }
      ], 3000);
      let html = aiResp.choices[0].message.content.trim();
      html = html.replace(/```html?/g, '').replace(/```/g, '');
      const fullHtml = `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>body{font-family:-apple-system,BlinkMacSystemFont,sans-serif;max-width:700px;margin:40px auto;padding:0 20px;line-height:1.7;color:#1a1a2e}h1{font-size:24px;border-bottom:2px solid #7c3aed;padding-bottom:8px}h2{font-size:18px;color:#7c3aed;margin-top:20px}table{width:100%;border-collapse:collapse;margin:16px 0}th,td{padding:8px 12px;border:1px solid #ddd;text-align:left}th{background:#f5f3ff}.footer{margin-top:32px;padding-top:16px;border-top:1px solid #ddd;font-size:11px;color:#888}</style></head><body>${html}<div class="footer">Generated by TableTurn · ${new Date().toLocaleDateString()}</div></body></html>`;
      return res.json({ html: fullHtml, templateId: 'custom', title: 'Custom Document' });
    }
